"""Train and evaluate the Utama et al. Attention-BiLSTM baseline."""
...
from src.data.utama_dataset import collect_samples, split_train_val
from src.models.attention_bilstm import AttentionBiLSTM, model_summary
...
def train_utama(
    ...
    """Train one paper-configured model and return validation metrics."""
    _set_seed(seed)
    ...
    model = AttentionBiLSTM(input_dim=80, num_classes=num_classes, hidden_size=hidden_size, dropout=dropout).to(torch_device)
                ...
                torch.save(
                    {
                        ...
                        "model_config": {"input_dim": 80, "num_classes": num_classes, "hidden_size": hidden_size, "dropout": dropout},
                        "epoch": epoch,
                        "val_acc": val_acc,
                        "val_loss": val_loss,
                        "seed": seed,
    ...
    summary = {
        ...
        "model_summary": model_summary(model, (batch_size, 40 if num_classes == 10 else 60, 80)),
...
def main() -> None:
    ...
    parser.add_argument("--precomputed-root", type=Path, default=Path("precomputed_utama"))
