"""Behavior checks for AripinV2 model and training helpers."""
...
from src.training.train_aripin_v2 import _collate_fn, augment_training_batch, train_aripin_v2


def test_model_contract() -> None:
    cases = [(10, 30, 3_854_218), (4, 40, 3_853_060)]
    for num_classes, steps, expected_params in cases:
        model = AripinV2(num_classes)
...
def test_collate_and_smoke_training() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        summary = train_aripin_v2(
            ...
            run_name="test_aripin_v2",
        )
        assert summary["n_params"] == 3_852_674
        assert Path(summary["checkpoint"]).exists()
        assert Path(tmp, "logs", "test_aripin_v2.csv").exists()
        assert Path(tmp, "logs", "test_aripin_v2.json").exists()
